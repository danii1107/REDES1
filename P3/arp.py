...
def processARPRequest(data:bytes,MAC:bytes)->None:
    ...
    ip_origen = data[8:12]
    ip_target = data[18:22]

    my_ip = struct.pack('!I', myIP)
    if ip_target != my_ip:
        return
...
def process_arp_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes,srcMac:bytes) -> None:
    '''
        ...
        Retorno: Ninguno
    '''
    cabecera_comun = data[:6]

    if cabecera_comun != ARPHeader:
        print("la cabecera es incorrecta")
    opcode = data[6:8]
...
def initARP(interface:str) -> int:
    ...
    aux = ARPResolution(myIP)
    if aux != None:
        print("ARP ya inicializado")
        return 1
